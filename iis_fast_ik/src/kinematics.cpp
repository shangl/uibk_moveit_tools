...
bool Kinematics::computeIK(const string &arm,
    ...
    ROS_DEBUG("IK request received for group '%s' and tip link '%s'", ik_group.c_str(), tip_link.c_str());
    // connect to ik group
    ...
    // compute result
    if(state.setFromIK(jnt_model_group, goal, tip_link, attempts, timeout, boost::bind(&Kinematics::collisionCheckCallback, this, _1, _2, _3))) {
...
bool Kinematics::computeFK(const string &arm,
	...
	const Eigen::Affine3d &eef_state = state.getGlobalLinkTransform(tip_link);
